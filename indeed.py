...
def parse_html(job) -> dict:
    job_dict = {'title': job.find('h2 > a')[0].text,
                'link': 'https://in.indeed.com/jobs?q=' + job.find('h2 > a')[0].attrs['data-jk'],
...
def main():
    ...
    baseurl = 'https://in.indeed.com/jobs?q=python+developer&l=Noida '

    s = HTMLSession()
    results = []
    url = baseurl + f'/jobs?q={job_search}&l=Noida'
    while True:
